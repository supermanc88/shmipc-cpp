...
func main() {
	...
	commandArgs := []string{"test", "-overlay", overlayPath, "-run", "^Test((ControlHeader|SharedMemoryMetadata|FallbackData|QueueLayout|BufferLayout)Golden|(BufferPool|Queue|V2Handshake|V2ClientSession)Interop)$", "-count=1", "-v", "-timeout=20s", "."}
	if compileOutput != "" {
