...
- `shmipc.v2_multiplexed_session`：Linux 上验证 client ID 2/3/4、并发首包、server Accept、独立双向消息、无 ACK close 的两个主动关闭方向与 Stream/Session 资源生命周期。
- `shmipc.go_protocol_oracle`：除控制协议与布局外，调用真实 C++ helpers 双向传递 slice chain/queue elements，在 Linux 验证两个方向的 v2 握手，以及 C++ client/server 两个方向的单 Stream 和 3 Stream 数据与关闭。
