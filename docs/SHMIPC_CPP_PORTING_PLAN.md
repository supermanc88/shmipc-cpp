...
| 当前阶段 | M2 与 M3 `S-0301..0303` 已完成；进入 `S-0304` Go client→C++ server |
...
1. 进入 `S-0304`，实现 Go client→C++ server 及多消息/关闭路径。
2. 以真实 Go client 验证连续消息、跨 slice、Polling 与双向关闭，并完成远端/Sanitizer/云端门禁。
