...
| 当前阶段 | M0 执行中；S-0001 已验收，S-0002 Linux CI/Sanitizer 首轮云端矩阵已验证，下一步进入 S-0003 |
...
- `S-0201`：RAII mmap/memfd/file mapping。
...
- `S-0301`：Unix/TCP 控制 transport 和 epoll dispatcher。
...
6. 给出用户可重复的人工/集成验收步骤与预期结果。
7. 用户验收后记录 evidence，创建候选提交；远程 CI 通过后标记完成。
...
1. 建立 `docs/feature-matrix.md`、首批 ADR 和 `docs/regression-test-guide.md`。
2. 执行 `S-0003`：固定 Go oracle，生成 control-header golden 并在 C++ 测试中消费。
3. M0 验收后进入 M1，不跨越 counter offset 决策门。
