...
| 当前阶段 | M2、M3 已完成；进入 M4 `S-0401` v3 版本协商状态机 |
...
- `S-0305a`（已验证）：client 连续 Open ID 2/3/4、server 首包 Accept、per-Stream 并发路由与无 ACK close；run `32204938990` 七项门禁成功。
- `S-0305b`（已验证）：persistent deadline、queue-full retry/close fallback、句柄释放/Session failure 时的路由回收与错误传播；run `32204938990` 七项门禁成功。
...
1. 进入 `S-0401` v3 版本协商状态机；继续保留多 Stream、deadline、关闭与 Session 断开矩阵作为回归。
