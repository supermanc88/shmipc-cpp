...
组合 protocol、transport 与 shm 子系统形成可执行的会话初始化和数据路径。当前实现 v2 文件路径握手、独立 v3 版本协商、client/server 单 Stream 基线，以及带 deadline、queue-full retry 和错误扇出的 client-originated 多 Stream Session；v3 FD 传递与数据 fallback 仍由后续切片补充。
