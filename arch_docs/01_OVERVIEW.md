...
- C++ v2/v3 共用多路数据面入口：`src/core/v2_multiplexed_session.hpp`。连接 state 以 variant 持有文件模式 `V2SharedMemory` 或 memfd 模式 `V3SharedMemory`，所有 Polling/FallbackData/StreamClose 帧使用握手对应版本。buffer 分配返回 `no_buffer` 时以 FallbackData 走控制连接并永久标记该 Stream；queue full 不触发数据 fallback。
...
- `S-0402` 验证：完整 v3 memfd 握手通过本机 Debug/ASan+UBSan/TSan、远端 GCC 8.5 Debug/ASan 17/17；固定 Go `newSession` 两方向普通 100 轮、ASan helper 20 轮通过；提交 `568817c` 的 run `32209295664` 七项门禁成功。
