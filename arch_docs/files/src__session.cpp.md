...
实现公共 client API 的连接与同步操作适配层：归一化内部错误，校验公共配置，选择 v2 file 或 v3 memfd handshake，并集中拥有内部 Session 与 epoll dispatcher。异步执行器实现在 `src/callback.cpp`。
...
5. `open_stream` 将内部 move-only Stream 包入 shared public PImpl；send/receive/deadline 做状态归一化与转发，close 与异步 control 协调等待。
...
- close/共享 PImpl ownership：`src/session.cpp:223-280`、`src/public/session_impl.hpp:17-27`。
